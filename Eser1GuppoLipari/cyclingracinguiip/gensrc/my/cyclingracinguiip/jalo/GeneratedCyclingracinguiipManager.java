...
public abstract class GeneratedCyclingracinguiipManager extends Extension
{
	...
	public Manager createManager(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( CyclingracinguiipConstants.TC.MANAGER );
	...
	public Team createTeam(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( CyclingracinguiipConstants.TC.TEAM );
			return (Team)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
	...
	public Team createTeam(final Map attributeValues)
	{
