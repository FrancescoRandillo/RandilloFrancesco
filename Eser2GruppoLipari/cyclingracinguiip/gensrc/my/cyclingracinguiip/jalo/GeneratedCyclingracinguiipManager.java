...
public abstract class GeneratedCyclingracinguiipManager extends Extension
{
	...
	public CyclistTeam createCyclistTeam(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( CyclingracinguiipConstants.TC.CYCLISTTEAM );
	...
	public Person createPerson(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( CyclingracinguiipConstants.TC.PERSON );
